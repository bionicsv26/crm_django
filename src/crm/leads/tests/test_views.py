...
class LeadCreateViewTest(LeadMixinViewTest, TestCase):
    ...
    def test_success_url(self):
        """
        Тест проверяет, что после успешного создания нового клиента происходит переход на список клиентов.
        """
        form_data = {
            'first_name': 'Андрей',
            'last_name': 'Андреев',
        ...
        response = self.client.post(reverse_lazy('crm.leads:leads_create'), data=form_data, follow=True)
    ...
    def test_with_permission_add_lead(self):
        """Тест проверяет, что с разрешением add_lead пользователь может создавать новых клиентов."""
        response = self.client.get(reverse('crm.leads:leads_create'))
...
class LeadListViewTest(LeadMixinViewTest, TestCase):
    ...
    def test_with_permission_view_lead(self):
        """Тест проверяет, что с разрешением view_lead пользователь может видеть список потенциальных клиентов."""
        response = self.client.get(reverse('crm.leads:leads_list'))
        self.assertEqual(response.status_code, 200)
...
class LeadDetailViewTest(LeadMixinViewTest, TestCase):
    ...
    def test_used_correct_template(self):
        """Тест проверяет, что используется корректный шаблон."""
        response = self.client.get(reverse('crm.leads:leads_detail', kwargs={'pk': self.lead.pk}))
        self.assertTemplateUsed(response, 'leads/leads-detail.html')

    def test_with_permission_view_lead(self):
        """Тест проверяет, что с разрешением view_lead пользователь может видеть детальное описание клиента."""
        response = self.client.get(reverse('crm.leads:leads_detail', kwargs={'pk': self.lead.pk}))
        self.assertEqual(response.status_code, 200)
...
class LeadUpdateViewTest(LeadMixinViewTest, TestCase):
    ...
    def test_success_update_lead(self):
        """
        Тест проверяет, что после успешного обновления данных компании пользователь направляется на список клиентов.
        """
        update_data = {
            'first_name': 'Андрей',
            'last_name': 'Андреев',
        ...
        response = self.client.post(reverse_lazy('crm.leads:leads_edit', kwargs={'pk': self.lead.pk}),
                                    data=update_data)
    ...
    def test_used_correct_template(self):
        """Тест проверяет, что используется корректный шаблон."""
        response = self.client.get(reverse('crm.leads:leads_edit', kwargs={'pk': self.lead.pk}))
        self.assertTemplateUsed(response, 'leads/leads-edit.html')

    def test_with_permission_change_lead(self):
        """Тест проверяет, что с разрешением change_lead пользователь может изменять данные клиента."""
        response = self.client.get(reverse('crm.leads:leads_edit', kwargs={'pk': self.lead.pk}))
        self.assertEqual(response.status_code, 200)
...
class LeadDeleteViewTest(LeadMixinViewTest, TestCase):
    ...
    def test_success_delete_lead(self):
        """Тест проверяет, что после успешного удаления клиента пользователь направляется на список клиентов."""
        lead_count = Lead.objects.count()
        response = self.client.post(reverse_lazy('crm.leads:leads_delete', kwargs={'pk': self.lead.pk}))
...
class LeadTransferToActiveViewTest(LeadMixinViewTest, TestCase):
    ...
    def test_product_id_cached_and_redirects(self):
        ...
        response = self.client.get(reverse('crm.leads:leads_to_active', kwargs={'pk': self.lead.pk}))
        cached_lead_id = cache.get('lead_id')
    ...
    def test_without_permission_can_transfer_to_active(self):
        """Тест проверяет, что без разрешения can_transfer_to_active пользователь получает ошибку 403."""
        self.user.user_permissions.remove(self.permission)
        response = self.client.get(reverse('crm.leads:leads_to_active', kwargs={'pk': self.lead.pk}))
        self.assertEqual(response.status_code, 403)
...
class LeadTransferToContractViewTest(LeadMixinViewTest, TestCase):
    ...
    def test_product_id_cached_and_redirects(self):
        ...
        response = self.client.get(reverse('crm.leads:leads_to_contract', kwargs={'pk': self.lead.pk}))
        cached_lead_id = cache.get('lead_id')
        self.assertEqual(cached_lead_id, self.lead.pk)
    ...
    def test_with_permission_can_transfer_to_contract(self):
        """Тест проверяет, что с разрешением can_transfer_to_contract пользователь может создавать контракт."""
        response = self.client.get(reverse('crm.leads:leads_to_contract', kwargs={'pk': self.lead.pk}))
        self.assertEqual(response.status_code, 302)

    def test_without_permission_can_transfer_to_contract(self):
        """Тест проверяет, что без разрешения can_transfer_to_contract пользователь получает ошибку 403."""
        self.user.user_permissions.remove(self.permission)
        response = self.client.get(reverse('crm.leads:leads_to_contract', kwargs={'pk': self.lead.pk}))
        self.assertEqual(response.status_code, 403)
